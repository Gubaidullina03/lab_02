...
Если все хорошо, плавно перезапускаем Nginx, чтобы он применил изменения
...
1.Терминал 1 ("Кухня"). Убедимся, что в одном терминале у вас запущено Flask приложение. Вы будем видеть строки вроде * Running on http://127.0.0.1:5000. Закрывайте его не будем.
2.Терминал 2 ("Клиент"). Откроем второй, чистый терминал. Все команды ниже мы
будем выполнять именно в нем.
3.Nginx. Он будет работать в фоне после команды sudo systemctl restart nginx.
